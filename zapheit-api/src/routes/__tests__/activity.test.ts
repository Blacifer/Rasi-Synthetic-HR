...
describe('GET /events', () => {
  ...
  it('includes audit-backed unified events when filtering by source type', async () => {
    mockRest
      .mockResolvedValueOnce([
        {
          id: 'audit-approval-only',
          action: 'approval.partial_approval',
          resource_type: 'approval_request',
          ...
          details: {
            unified_activity_event: {
              id: 'approval-partial-2',
              type: 'approval',
              status: 'needs_policy',
              tone: 'warn',
              at: '2026-04-30T00:00:04.000Z',
              title: 'Approval recorded: runtime job',
              detail: '1/2 approvals recorded; waiting for the remaining reviewer.',
              route: 'approvals',
            },
          },
        },
    ...
    const { statusCode, body } = await invokeActivityRoute('/events', { limit: '20', type: 'approval' });
    ...
    expect(body.data.events[0]).toMatchObject({
      ...
      title: 'Approval recorded: runtime job',
      route: 'approvals',
      evidenceRef: 'audit-approval-only',
    });
    expect(mockRest).toHaveBeenCalledTimes(2);
    expect(mockRest).toHaveBeenCalledWith(
      'mock-jwt',
