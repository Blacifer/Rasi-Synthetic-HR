...
export const ConditionNode = memo(({ data, selected }: NodeProps<ConditionNodeData>) => (
  <div
    className={`bg-white dark:bg-gray-800 border-2 rounded-xl shadow-md px-4 py-3 min-w-[180px] max-w-[240px] ${
      selected ? 'border-amber-500' : 'border-amber-300 dark:border-amber-700'
    }`}
  >
    <Handle type="target" position={Position.Top} className="!bg-amber-400" />
    <div className="flex items-center gap-2 mb-1">
      <GitBranch className="w-4 h-4 text-amber-500 shrink-0" />
      <span className="text-xs font-semibold text-amber-700 dark:text-amber-300 uppercase tracking-wide">Condition</span>
    </div>
    <p className="text-sm font-medium text-gray-800 dark:text-gray-100 truncate">{data.label || 'If / Else'}</p>
    {data.condition && (
